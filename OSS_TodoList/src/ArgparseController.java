...
public class ArgparseController {
	ArgparseController(String args[]) {
		...
		try {
			...
			if (ns.getBoolean("version") != null && ns.getBoolean("version")) {
				return;
			} 
			else if (ns.getBoolean("help") != null && ns.getBoolean("help")) {
				return;
			} 
			...
			else if (ns.getBoolean("add")) {
				view.inputForAdd();
			}
			else if (ns.getBoolean("list")) {
				view.chooseShowOption();
			}
			else if (ns.getBoolean("modify")) {
				view.inputToModify();
			}
			else if (ns.getBoolean("remove")) {
				view.inputToRemove();
			}
			else if (ns.getBoolean("order")) {
				view.setOrderOption();
			}
